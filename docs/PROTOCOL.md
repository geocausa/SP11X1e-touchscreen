...
3. Read the class-3 reset response generated by the reset operation.
4. Send the E2 device-descriptor request.
5. Class 7 returns the 24-byte HID-over-SPI device descriptor.
...
The transport classes follow the Microsoft HID-over-SPI specification:
class 1 is unsolicited HID input, class 3 is a reset response, class 7 is the
device descriptor, and class 8 is the report descriptor. GPIO gating remains
mandatory for unsolicited input; solicited output-report responses are polled
with a bounded one-second timeout.
