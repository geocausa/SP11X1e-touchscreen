...
# Build an isolated initramfs for the Phase 77 recovery experiment. The saved
# Phase 75 entry, deployed Phase 76 image, DTB, and root-filesystem modules are
# never replaced. The new entry enables behavior_v2 and reset_recovery_v2.
...
target_assets=/boot/sp11-7.1.3-phase77-recovery
target_grub=/etc/grub.d/64_sp11_713_phase77_recovery
...
if [ -e "$target_assets" ] || [ -e "$target_grub" ]; then
	echo "Phase 77 target already exists; refusing to overwrite it" >&2
	exit 1
fi
...
for file in "$built_client" "$built_controller" "$built_gpi" \
	...
	"$root/boot/64_sp11_713_phase77_recovery"; do
	if [ ! -f "$file" ]; then
...
for parameter in behavior_v2 reset_recovery_v2; do
...
mkinitramfs -d "$work/initramfs-tools" \
	-o "$work/initrd.img-$release-phase77" "$release"
unmkinitramfs "$work/initrd.img-$release-phase77" "$work/extracted"
...
if [ -z "$embedded_client" ] ||
	...
	echo "new initramfs does not contain the exact Phase 77 client" >&2
	exit 1
fi
for parameter in behavior_v2 reset_recovery_v2; do
...
install -m 0600 "$work/initrd.img-$release-phase77" \
	"$target_assets/initrd.img-$release-phase77"
install -m 0755 "$root/boot/64_sp11_713_phase77_recovery" "$target_grub"

commit=$(git -C "$root" rev-parse HEAD)
{
	echo "Phase 77 source commit: $commit"
...
if ! grep -q -- "--id 'sp11-phase77-recovery'" /boot/grub/grub.cfg ||
	! grep -q "sp11_entry=7.1.3-phase77-recovery" /boot/grub/grub.cfg ||
	! grep -q "mshw0485_touch.behavior_v2=1" /boot/grub/grub.cfg ||
	! grep -q "mshw0485_touch.reset_recovery_v2=1" /boot/grub/grub.cfg; then
	echo "generated GRUB configuration is missing the Phase 77 entry" >&2
	exit 1
fi
grub-reboot sp11-phase77-recovery
deployment_complete=1

echo "installed isolated Phase 77 recovery test: sp11-phase77-recovery"
