...
# Build an isolated initramfs for the Phase 81 ready-line quiesce experiment.
...
target_assets=/boot/sp11-7.1.3-phase81-ready-quiesce
target_grub=/etc/grub.d/68_sp11_713_phase81_ready_quiesce
...
if [ -e "$target_assets" ] || [ -e "$target_grub" ]; then
	echo "Phase 81 target already exists; refusing to overwrite it" >&2
	exit 1
fi
...
for file in "$built_client" "$built_controller" "$built_gpi" \
	...
	"$root/boot/68_sp11_713_phase81_ready_quiesce"; do
	if [ ! -f "$file" ]; then
...
for parameter in behavior_v2 reset_recovery_v2 host_fault_recovery ready_quiesce; do
...
mkinitramfs -d "$work/initramfs-tools" \
	-o "$work/initrd.img-$release-phase81" "$release"
unmkinitramfs "$work/initrd.img-$release-phase81" "$work/extracted"
...
if [ -z "$embedded_client" ] ||
	...
	echo "new initramfs does not contain the exact Phase 81 client" >&2
	exit 1
fi
for parameter in behavior_v2 reset_recovery_v2 host_fault_recovery ready_quiesce; do
...
install -m 0600 "$work/initrd.img-$release-phase81" \
	"$target_assets/initrd.img-$release-phase81"
install -m 0755 "$root/boot/68_sp11_713_phase81_ready_quiesce" "$target_grub"

commit=$(git -C "$root" rev-parse HEAD)
{
	echo "Phase 81 source commit: $commit"
...
if ! grep -q -- "--id 'sp11-phase81-ready-quiesce'" /boot/grub/grub.cfg ||
	! grep -q "sp11_entry=7.1.3-phase81-ready-quiesce" /boot/grub/grub.cfg ||
	...
	! grep -q "mshw0485_touch.ready_quiesce=1" /boot/grub/grub.cfg; then
	echo "generated GRUB configuration is missing the Phase 81 entry" >&2
	exit 1
fi
grub-reboot sp11-phase81-ready-quiesce
deployment_complete=1

echo "installed isolated Phase 81 ready-line quiesce: sp11-phase81-ready-quiesce"
