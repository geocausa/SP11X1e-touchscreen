...
# Build an isolated initramfs for the Phase 78 reset-storm circuit breaker.
# Existing Phase 75 through Phase 77 assets and root-filesystem modules are
# never replaced.
...
target_assets=/boot/sp11-7.1.3-phase78-storm-breaker
target_grub=/etc/grub.d/65_sp11_713_phase78_storm_breaker
...
if [ -e "$target_assets" ] || [ -e "$target_grub" ]; then
	echo "Phase 78 target already exists; refusing to overwrite it" >&2
	exit 1
fi
...
for file in "$built_client" "$built_controller" "$built_gpi" \
	...
	"$root/boot/65_sp11_713_phase78_storm_breaker"; do
	if [ ! -f "$file" ]; then
...
for parameter in behavior_v2 reset_recovery_v2 reset_storm_breaker; do
...
mkinitramfs -d "$work/initramfs-tools" \
	-o "$work/initrd.img-$release-phase78" "$release"
unmkinitramfs "$work/initrd.img-$release-phase78" "$work/extracted"
...
if [ -z "$embedded_client" ] ||
	...
	echo "new initramfs does not contain the exact Phase 78 client" >&2
	exit 1
fi
for parameter in behavior_v2 reset_recovery_v2 reset_storm_breaker; do
...
install -m 0600 "$work/initrd.img-$release-phase78" \
	"$target_assets/initrd.img-$release-phase78"
install -m 0755 "$root/boot/65_sp11_713_phase78_storm_breaker" "$target_grub"

commit=$(git -C "$root" rev-parse HEAD)
{
	echo "Phase 78 source commit: $commit"
	...
	echo "storm breaker: mshw0485_touch.reset_storm_breaker=1"
...
if ! grep -q -- "--id 'sp11-phase78-storm-breaker'" /boot/grub/grub.cfg ||
	! grep -q "sp11_entry=7.1.3-phase78-storm-breaker" /boot/grub/grub.cfg ||
	! grep -q "mshw0485_touch.behavior_v2=1" /boot/grub/grub.cfg ||
	! grep -q "mshw0485_touch.reset_recovery_v2=1" /boot/grub/grub.cfg ||
	! grep -q "mshw0485_touch.reset_storm_breaker=1" /boot/grub/grub.cfg; then
	echo "generated GRUB configuration is missing the Phase 78 entry" >&2
	exit 1
fi
grub-reboot sp11-phase78-storm-breaker
deployment_complete=1

echo "installed isolated Phase 78 reset storm breaker: sp11-phase78-storm-breaker"
