...
name=SP11X1e-touchscreen-phase68-7.1.1-sp11-gpicmp1-aarch64
...
for module in gpi spi-geni-qcom g6ts_biosref; do
	...
	if [ ! -f "$path" ]; then
		echo "missing $path; build the Phase 68 matched set first" >&2
		exit 1
	fi
...
{
	echo "Surface Pro 11 MSHW0485 touchscreen Phase 68"
	echo "source commit: $commit"
	echo "target vermagic: $expected_vermagic"
	echo "hardware: Surface Pro 11 OLED / Snapdragon X Elite"
	echo "recovery: Phase 55-65 hardware-proven minimal sequence"
	echo
