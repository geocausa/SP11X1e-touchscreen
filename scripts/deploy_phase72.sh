...
source_assets=/boot/sp11-7.1.1-phase70
target_assets=/boot/sp11-7.1.1-phase72
target_grub=/etc/grub.d/59_sp11_711_phase72_config
...
if [ -e "$target_assets" ] || [ -e "$target_grub" ]; then
	echo "Phase 72 target already exists; refusing to overwrite it" >&2
	exit 1
fi
for file in "$built_client" "$built_controller" "$built_gpi" \
	...
	"$source_assets/sp11-7.1.1-phase70-hybrid.dtb"; do
	if [ ! -f "$file" ]; then
...
if [ "$(modinfo -F srcversion "$built_controller")" != \
	...
	echo "installed GENI controller does not match the Phase 72 build" >&2
	exit 1
fi
if [ "$(modinfo -F srcversion "$built_gpi")" != \
	"$(modinfo -F srcversion "$installed_gpi")" ]; then
	echo "installed GPI module does not match the Phase 72 build" >&2
	exit 1
fi
...
module_swapped=1
depmod -a "$release"
mkinitramfs -o "$work/initrd.img-$release-phase72" "$release"
unmkinitramfs "$work/initrd.img-$release-phase72" "$work/extracted"
...
if [ -z "$embedded_client" ] ||
	...
	echo "new initramfs does not contain the Phase 72 client" >&2
	exit 1
fi
...
install -m 0644 "$source_assets/sp11-7.1.1-phase70-hybrid.dtb" \
	"$target_assets/sp11-7.1.1-phase72-hybrid.dtb"
install -m 0600 "$work/initrd.img-$release-phase72" \
	"$target_assets/initrd.img-$release-phase72"
install -m 0755 "$root/boot/59_sp11_711_phase72_config" "$target_grub"
...
{
	echo "Phase 72 source commit: $commit"
...
if ! grep -q -- "--id 'sp11-phase72'" /boot/grub/grub.cfg ||
	! grep -q "sp11_entry=7.1.1-phase72" /boot/grub/grub.cfg ||
	! grep -q "g6ts_biosref.mode_config_fix=1" /boot/grub/grub.cfg; then
	echo "generated GRUB configuration is missing the Phase 72 entry" >&2
	exit 1
fi
grub-reboot sp11-phase72

echo "installed isolated Phase 72 entry: sp11-phase72"
echo "saved GRUB default was not changed; next boot only selects Phase 72"
