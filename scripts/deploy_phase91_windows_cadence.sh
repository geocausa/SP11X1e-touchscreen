...
# Build a separate initramfs for Phase 90 plus measured Windows response
# cadence. Existing boot assets and installed modules are restored exactly.
...
target_assets=/boot/sp11-7.1.3-phase91-windows-cadence
target_grub=/etc/grub.d/77_sp11_713_phase91_windows_cadence
boot_source=$root/boot/77_sp11_713_phase91_windows_cadence
...
if [ "$(uname -r)" != "$release" ]; then
	echo "boot the $release Phase 75/90 baseline before deployment" >&2
	exit 1
fi
if [ -e "$target_assets" ] || [ -e "$target_grub" ]; then
	echo "Phase 91 target already exists; refusing to overwrite it" >&2
	exit 1
fi
...
for forbidden in \
	...
	"gpi.sp11_qspi_linux_link=1"; do
	if grep -q "$forbidden" "$boot_source"; then
		echo "Phase 91 unexpectedly enables lower-stack override: $forbidden" >&2
		exit 1
	fi
done
for required in \
	"mshw0485_touch.parity_linux_power=1" \
	"mshw0485_touch.windows_read_cadence=1"; do
	if ! grep -q "$required" "$boot_source"; then
		echo "Phase 91 is missing isolated option: $required" >&2
		exit 1
	fi
done
...
for parameter in windows_init_parity parity_linux_power windows_read_cadence parity_display_bitmap parity_stitching_flag parity_hinge_angle parity_fast_host_id parity_report56_identity parity_report56_flag parity_cfu_inventory parity_cfu_offer parity_heat_input behavior_v2 host_fault_recovery ready_quiesce; do
...
mkinitramfs -d "$work/initramfs-tools" \
	-o "$work/initrd.img-$release-phase91" "$release"
unmkinitramfs "$work/initrd.img-$release-phase91" "$work/extracted"
...
if [ -z "$embedded_client" ] ||
	...
	echo "new initramfs does not contain the exact Phase 91 client" >&2
	exit 1
fi
for parameter in windows_init_parity parity_linux_power windows_read_cadence parity_cfu_inventory parity_heat_input behavior_v2 host_fault_recovery ready_quiesce; do
...
install -m 0600 "$work/initrd.img-$release-phase91" \
	"$target_assets/initrd.img-$release-phase91"
install -m 0755 "$boot_source" "$target_grub"

commit=$(git -C "$root" rev-parse HEAD)
{
	echo "Phase 91 source commit: $commit"
	echo "source worktree: tracked files clean"
	echo "target kernel: $release"
	echo "transport/power: Phase 75 hardware-proven paths"
	echo "chronology: Windows init + bounded CFU inventory + Heat"
	echo "response cadence: Windows 490-550 us header/body + one response per IRQ"
	echo "contact profile: Phase 76 Windows-derived behavior"
	echo "recovery: host-fault recovery + ready-line quiesce"
	echo "firmware payload path: absent"
	echo "saved default: sp11-phase75-identity"
...
for token in "--id 'sp11-phase91-windows-cadence'" \
	"sp11_entry=7.1.3-phase91-windows-cadence" \
	...
	"mshw0485_touch.ready_quiesce=1"; do
	if ! grep -q -- "$token" /boot/grub/grub.cfg; then
		echo "generated GRUB configuration is missing Phase 91 token: $token" >&2
		exit 1
	fi
done
...
echo "installed isolated Phase 91 Windows response-cadence test"
echo "the saved Phase 75 default remains unchanged; next boot only is armed"
