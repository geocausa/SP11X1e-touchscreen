...
# Build a separate initramfs for the Windows upper chronology over the full
# Phase 75 Linux controller/GPI transport. Existing boot assets and installed
# root-filesystem modules are restored exactly.
...
target_assets=/boot/sp11-7.1.3-phase89-linux-transport-heat
target_grub=/etc/grub.d/75_sp11_713_phase89_linux_transport_heat
boot_source=$root/boot/75_sp11_713_phase89_linux_transport_heat
...
if [ "$(uname -r)" != "$release" ]; then
	echo "boot the $release Phase 75/88 baseline before deployment" >&2
	exit 1
fi
if [ -e "$target_assets" ] || [ -e "$target_grub" ]; then
	echo "Phase 89 target already exists; refusing to overwrite it" >&2
	exit 1
fi
...
for forbidden in \
	...
	if grep -q "$forbidden" "$boot_source"; then
		echo "Phase 89 unexpectedly enables lower-stack override: $forbidden" >&2
		exit 1
	fi
done
...
for parameter in windows_init_parity parity_display_bitmap parity_stitching_flag parity_hinge_angle parity_fast_host_id parity_report56_identity parity_report56_flag parity_cfu_inventory parity_cfu_offer parity_heat_input behavior_v2 host_fault_recovery ready_quiesce; do
...
mkinitramfs -d "$work/initramfs-tools" \
	-o "$work/initrd.img-$release-phase89" "$release"
unmkinitramfs "$work/initrd.img-$release-phase89" "$work/extracted"
...
if [ -z "$embedded_client" ] ||
	...
	echo "new initramfs does not contain the exact Phase 89 client" >&2
	exit 1
fi
for parameter in windows_init_parity parity_cfu_inventory parity_heat_input behavior_v2 host_fault_recovery ready_quiesce; do
...
install -m 0600 "$work/initrd.img-$release-phase89" \
	"$target_assets/initrd.img-$release-phase89"
install -m 0755 "$boot_source" "$target_grub"

commit=$(git -C "$root" rev-parse HEAD)
{
	echo "Phase 89 source commit: $commit"
...
for token in "--id 'sp11-phase89-linux-transport-heat'" \
	"sp11_entry=7.1.3-phase89-linux-transport-heat" \
	...
	if ! grep -q -- "$token" /boot/grub/grub.cfg; then
		echo "generated GRUB configuration is missing Phase 89 token: $token" >&2
		exit 1
	fi
done
...
echo "installed isolated Phase 89 Linux-transport/Windows-chronology/Heat test"
