...
target_assets=/boot/sp11-7.1.3-phase87-linux-link-heat
target_grub=/etc/grub.d/72_sp11_713_phase87_linux_link_heat
...
if [ -e "$target_assets" ] || [ -e "$target_grub" ]; then
	echo "Phase 87 target already exists; refusing to overwrite it" >&2
	exit 1
fi
...
for file in "$built_client" "$built_controller" "$built_gpi" \
	...
	"$root/boot/72_sp11_713_phase87_linux_link_heat"; do
	if [ ! -f "$file" ]; then
...
mkinitramfs -d "$work/initramfs-tools" \
	-o "$work/initrd.img-$release-phase87" "$release"
unmkinitramfs "$work/initrd.img-$release-phase87" "$work/extracted"
...
if [ -z "$embedded_client" ] ||
	...
	echo "new initramfs does not contain the exact Phase 87 client" >&2
	exit 1
fi
...
install -m 0600 "$work/initrd.img-$release-phase87" \
	"$target_assets/initrd.img-$release-phase87"
install -m 0755 "$root/boot/72_sp11_713_phase87_linux_link_heat" "$target_grub"

commit=$(git -C "$root" rev-parse HEAD)
{
	echo "Phase 87 source commit: $commit"
	echo "source worktree: tracked files clean"
	echo "target kernel: $release"
	echo "transport: Windows-init QSPI + captured QGPI rings + Linux LINK"
...
for token in "--id 'sp11-phase87-linux-link-heat'" \
	...
	if ! grep -q -- "$token" /boot/grub/grub.cfg; then
		echo "generated GRUB configuration is missing Phase 87 token: $token" >&2
		exit 1
	fi
done
...
echo "installed isolated Phase 87 Windows-init/CFU/Heat test"
