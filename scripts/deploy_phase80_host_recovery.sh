...
# Build an isolated initramfs for the Phase 80 host-fault recovery.
...
target_assets=/boot/sp11-7.1.3-phase80-host-recovery
target_grub=/etc/grub.d/67_sp11_713_phase80_host_recovery
...
if [ -e "$target_assets" ] || [ -e "$target_grub" ]; then
	echo "Phase 80 target already exists; refusing to overwrite it" >&2
	exit 1
fi
...
for file in "$built_client" "$built_controller" "$built_gpi" \
	...
	"$root/boot/67_sp11_713_phase80_host_recovery"; do
	if [ ! -f "$file" ]; then
...
for parameter in behavior_v2 reset_recovery_v2 host_fault_recovery; do
...
mkinitramfs -d "$work/initramfs-tools" \
	-o "$work/initrd.img-$release-phase80" "$release"
unmkinitramfs "$work/initrd.img-$release-phase80" "$work/extracted"
...
if [ -z "$embedded_client" ] ||
	...
	echo "new initramfs does not contain the exact Phase 80 client" >&2
	exit 1
fi
for parameter in behavior_v2 reset_recovery_v2 host_fault_recovery; do
...
install -m 0600 "$work/initrd.img-$release-phase80" \
	"$target_assets/initrd.img-$release-phase80"
install -m 0755 "$root/boot/67_sp11_713_phase80_host_recovery" "$target_grub"

commit=$(git -C "$root" rev-parse HEAD)
{
	echo "Phase 80 source commit: $commit"
...
if ! grep -q -- "--id 'sp11-phase80-host-recovery'" /boot/grub/grub.cfg ||
	! grep -q "sp11_entry=7.1.3-phase80-host-recovery" /boot/grub/grub.cfg ||
	! grep -q "mshw0485_touch.behavior_v2=1" /boot/grub/grub.cfg ||
	! grep -q "mshw0485_touch.reset_recovery_v2=1" /boot/grub/grub.cfg ||
	! grep -q "mshw0485_touch.host_fault_recovery=1" /boot/grub/grub.cfg; then
	echo "generated GRUB configuration is missing the Phase 80 entry" >&2
	exit 1
fi
grub-reboot sp11-phase80-host-recovery
deployment_complete=1

echo "installed isolated Phase 80 host fault recovery: sp11-phase80-host-recovery"
