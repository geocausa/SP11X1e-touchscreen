...
target_assets=/boot/sp11-7.1.3-phase86-windows-heat
target_grub=/etc/grub.d/71_sp11_713_phase86_windows_heat
...
if [ -e "$target_assets" ] || [ -e "$target_grub" ]; then
	echo "Phase 86 target already exists; refusing to overwrite it" >&2
	exit 1
fi
...
for file in "$built_client" "$built_controller" "$built_gpi" \
	...
	"$root/boot/71_sp11_713_phase86_windows_heat"; do
	if [ ! -f "$file" ]; then
...
mkinitramfs -d "$work/initramfs-tools" \
	-o "$work/initrd.img-$release-phase86" "$release"
unmkinitramfs "$work/initrd.img-$release-phase86" "$work/extracted"
...
if [ -z "$embedded_client" ] ||
	...
	echo "new initramfs does not contain the exact Phase 86 client" >&2
	exit 1
fi
...
install -m 0600 "$work/initrd.img-$release-phase86" \
	"$target_assets/initrd.img-$release-phase86"
install -m 0755 "$root/boot/71_sp11_713_phase86_windows_heat" "$target_grub"

commit=$(git -C "$root" rev-parse HEAD)
{
	echo "Phase 86 source commit: $commit"
	echo "source worktree: tracked files clean"
	echo "target kernel: $release"
	echo "transport: Windows-init QSPI + captured QGPI ring layout"
...
for token in "--id 'sp11-phase86-windows-heat'" \
	...
	if ! grep -q -- "$token" /boot/grub/grub.cfg; then
		echo "generated GRUB configuration is missing Phase 86 token: $token" >&2
		exit 1
	fi
done
grub-reboot sp11-phase86-windows-heat
deployment_complete=1

echo "installed isolated Phase 86 Windows-init/CFU/Heat test"
