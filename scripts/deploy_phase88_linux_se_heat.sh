...
# Build a separate initramfs for the complete Windows-init/CFU/Heat laboratory
# path. Existing boot assets and root-filesystem modules are restored exactly.
...
target_assets=/boot/sp11-7.1.3-phase88-linux-se-heat
target_grub=/etc/grub.d/74_sp11_713_phase88_linux_se_heat
...
if [ "$(uname -r)" != "$release" ]; then
	echo "boot the $release Phase 75 baseline before deployment" >&2
	exit 1
fi
if [ -e "$target_assets" ] || [ -e "$target_grub" ]; then
	echo "Phase 88 target already exists; refusing to overwrite it" >&2
	exit 1
fi
...
for file in "$built_client" "$built_controller" "$built_gpi" \
	...
	"$root/boot/74_sp11_713_phase88_linux_se_heat"; do
	if [ ! -f "$file" ]; then
...
if ! modinfo -p "$built_gpi" | grep -q '^sp11_qspi_linux_link:'; then
	echo "GPI module does not expose sp11_qspi_linux_link" >&2
	exit 1
fi
...
mkinitramfs -d "$work/initramfs-tools" \
	-o "$work/initrd.img-$release-phase88" "$release"
unmkinitramfs "$work/initrd.img-$release-phase88" "$work/extracted"
...
if [ -z "$embedded_client" ] ||
	...
	echo "new initramfs does not contain the exact Phase 88 client" >&2
	exit 1
fi
...
install -m 0600 "$work/initrd.img-$release-phase88" \
	"$target_assets/initrd.img-$release-phase88"
install -m 0755 "$root/boot/74_sp11_713_phase88_linux_se_heat" "$target_grub"

commit=$(git -C "$root" rev-parse HEAD)
{
	echo "Phase 88 source commit: $commit"
	echo "source worktree: tracked files clean"
	echo "target kernel: $release"
	echo "transport: Linux GENI init + captured QGPI rings + Linux LINK"
...
for token in "--id 'sp11-phase88-linux-se-heat'" \
	"sp11_entry=7.1.3-phase88-linux-se-heat" \
	...
	if ! grep -q -- "$token" /boot/grub/grub.cfg; then
		echo "generated GRUB configuration is missing Phase 88 token: $token" >&2
		exit 1
	fi
done
...
echo "installed isolated Phase 88 Linux-SE/Windows-chronology/Heat test"
