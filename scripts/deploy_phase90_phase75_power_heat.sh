...
# Build a separate initramfs for the complete Windows upper chronology over
# Phase 75 transport and Phase 75 power/reset. Installed modules are restored.
...
target_assets=/boot/sp11-7.1.3-phase90-phase75-power-heat
target_grub=/etc/grub.d/76_sp11_713_phase90_phase75_power_heat
boot_source=$root/boot/76_sp11_713_phase90_phase75_power_heat
...
if [ "$(uname -r)" != "$release" ]; then
	echo "boot the $release Phase 75/89 baseline before deployment" >&2
	exit 1
fi
if [ -e "$target_assets" ] || [ -e "$target_grub" ]; then
	echo "Phase 90 target already exists; refusing to overwrite it" >&2
	exit 1
fi
...
for forbidden in \
	...
	if grep -q "$forbidden" "$boot_source"; then
		echo "Phase 90 unexpectedly enables lower-stack override: $forbidden" >&2
		exit 1
	fi
done
if ! grep -q "mshw0485_touch.parity_linux_power=1" "$boot_source"; then
	echo "Phase 90 does not select the isolated Phase 75 power path" >&2
	exit 1
fi
...
for parameter in windows_init_parity parity_linux_power parity_display_bitmap parity_stitching_flag parity_hinge_angle parity_fast_host_id parity_report56_identity parity_report56_flag parity_cfu_inventory parity_cfu_offer parity_heat_input behavior_v2 host_fault_recovery ready_quiesce; do
...
mkinitramfs -d "$work/initramfs-tools" \
	-o "$work/initrd.img-$release-phase90" "$release"
unmkinitramfs "$work/initrd.img-$release-phase90" "$work/extracted"
...
if [ -z "$embedded_client" ] ||
	...
	echo "new initramfs does not contain the exact Phase 90 client" >&2
	exit 1
fi
for parameter in windows_init_parity parity_linux_power parity_cfu_inventory parity_heat_input behavior_v2 host_fault_recovery ready_quiesce; do
...
install -m 0600 "$work/initrd.img-$release-phase90" \
	"$target_assets/initrd.img-$release-phase90"
install -m 0755 "$boot_source" "$target_grub"

commit=$(git -C "$root" rev-parse HEAD)
{
	echo "Phase 90 source commit: $commit"
	echo "source worktree: tracked files clean"
	echo "target kernel: $release"
	echo "transport: Phase 75 Linux GENI/GPI geometry and normal LINK"
	echo "power/reset: Phase 75 hardware-proven sequence"
...
for token in "--id 'sp11-phase90-phase75-power-heat'" \
	"sp11_entry=7.1.3-phase90-phase75-power-heat" \
	...
	if ! grep -q -- "$token" /boot/grub/grub.cfg; then
		echo "generated GRUB configuration is missing Phase 90 token: $token" >&2
		exit 1
	fi
done
...
echo "installed isolated Phase 90 Phase75-power/Windows-chronology/Heat test"
