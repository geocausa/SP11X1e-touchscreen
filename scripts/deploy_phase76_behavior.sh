...
# Build an isolated initramfs for the Phase 76 processing experiment. The
# deployed Phase 75 image, DTB, saved GRUB default, and root-filesystem modules
# are never replaced. Only the new entry enables mshw0485_touch.behavior_v2.
...
target_assets=/boot/sp11-7.1.3-phase76-behavior
target_grub=/etc/grub.d/63_sp11_713_phase76_behavior
...
if [ -e "$target_assets" ] || [ -e "$target_grub" ]; then
	echo "Phase 76 target already exists; refusing to overwrite it" >&2
	exit 1
fi
...
for file in "$built_client" "$built_controller" "$built_gpi" \
	...
	"$root/boot/63_sp11_713_phase76_behavior"; do
	if [ ! -f "$file" ]; then
...
if ! modinfo -p "$built_client" | grep -q '^behavior_v2:'; then
	echo "client module does not expose behavior_v2" >&2
	exit 1
fi
...
mkinitramfs -d "$work/initramfs-tools" \
	-o "$work/initrd.img-$release-phase76" "$release"
unmkinitramfs "$work/initrd.img-$release-phase76" "$work/extracted"
...
if [ -z "$embedded_client" ] ||
	...
	echo "new initramfs does not contain the exact Phase 76 client" >&2
	exit 1
fi
if ! modinfo -p "$embedded_client" | grep -q '^behavior_v2:'; then
	echo "embedded client does not expose behavior_v2" >&2
	exit 1
fi
...
install -m 0600 "$work/initrd.img-$release-phase76" \
	"$target_assets/initrd.img-$release-phase76"
install -m 0755 "$root/boot/63_sp11_713_phase76_behavior" "$target_grub"

commit=$(git -C "$root" rev-parse HEAD)
{
	echo "Phase 76 source commit: $commit"
	echo "source worktree: clean"
	echo "target kernel: $release"
	echo "transport/recovery baseline: Phase 75 unchanged"
	echo "behavior profile: mshw0485_touch.behavior_v2=1"
...
if ! grep -q -- "--id 'sp11-phase76-behavior'" /boot/grub/grub.cfg ||
	! grep -q "sp11_entry=7.1.3-phase76-behavior" /boot/grub/grub.cfg ||
	! grep -q "mshw0485_touch.behavior_v2=1" /boot/grub/grub.cfg; then
	echo "generated GRUB configuration is missing the Phase 76 entry" >&2
	exit 1
fi
grub-reboot sp11-phase76-behavior
deployment_complete=1

echo "installed isolated Phase 76 behavior test: sp11-phase76-behavior"
