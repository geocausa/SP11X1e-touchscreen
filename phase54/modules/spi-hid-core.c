...
static int spi_hid_power_down(struct spi_hid *shid)
{
	...
	if (shid->reset_gpio)
		gpiod_set_value_cansleep(shid->reset_gpio, 0);
	if (shid->power_gpio)
		gpiod_set_value_cansleep(shid->power_gpio, 0);
	else if (shid->pinctrl_sleep)
...
static int spi_hid_power_up(struct spi_hid *shid)
{
	...
	if (shid->power_gpio) {
		gpiod_set_value_cansleep(shid->power_gpio, 1);
		msleep(500);
		gpiod_set_value_cansleep(shid->reset_gpio, 1);
		msleep(300);
	} else {
	...
	for (attempt = 0; attempt < SPI_HID_MAX_RESET_ATTEMPTS; attempt++) {
		...
		if (attempt > 0) {
			if (shid->reset_gpio)
				...
				pinctrl_select_state(shid->pinctrl,
						     shid->pinctrl_reset);
			msleep(SPI_HID_RESET_ASSERT_MS);
			if (shid->reset_gpio)
				gpiod_set_value_cansleep(shid->reset_gpio, 1);
...
static int spi_hid_probe(struct spi_device *spi)
{
	...
	shid->power_gpio = devm_gpiod_get_optional(dev, "power",
						 GPIOD_OUT_LOW);
	if (IS_ERR(shid->power_gpio)) {
		ret = dev_err_probe(dev, PTR_ERR(shid->power_gpio),
				    "failed to get power GPIO\n");
		goto err_sysfs;
	}
	shid->reset_gpio = devm_gpiod_get_optional(dev, "reset",
						 GPIOD_OUT_LOW);
	if (IS_ERR(shid->reset_gpio)) {
		ret = dev_err_probe(dev, PTR_ERR(shid->reset_gpio),
				    "failed to get reset GPIO\n");
		goto err_sysfs;
	}
	if (!!shid->power_gpio != !!shid->reset_gpio) {
		ret = dev_err_probe(dev, -EINVAL,
				    "power and reset GPIOs must be paired\n");
		goto err_sysfs;
	}

	shid->pinctrl = shid->power_gpio ? NULL : devm_pinctrl_get(dev);
	...
	shid->pinctrl_reset = shid->power_gpio ? NULL :
		pinctrl_lookup_state(shid->pinctrl, "reset");
	...
	shid->pinctrl_active = shid->power_gpio ? NULL :
		pinctrl_lookup_state(shid->pinctrl, "active");
	...
	if (!shid->power_gpio) {
		shid->pinctrl_sleep = pinctrl_lookup_state(shid->pinctrl,
							 "sleep");
