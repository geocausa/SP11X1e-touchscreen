...
#define GENI_QSPI_OUTPUTS		GENMASK(3, 0)
...
static int spi_geni_init(struct spi_geni_master *mas)
{
	...
	if (spi->target) {
		...
		spi_slv_setup(mas);
	} else if (mas->biosref_qspi) {
		if (proto != GENI_SE_QSPI) {
			dev_err(mas->dev,
				"BIOS-reference QSPI requires resident protocol 9, got %u\n",
				proto);
			goto out_pm;
		}
	...
	case 0:
		...
		if (mas->biosref_qspi)
			writel(GENI_QSPI_OUTPUTS,
			       se->base + GENI_OUTPUT_CTRL);
		ret = 0;
		break;
	}
...
static int spi_geni_probe(struct platform_device *pdev)
{
	...
	mas->dev = dev;
	mas->biosref_qspi =
		device_property_read_bool(dev, "qcom,biosref-qspi");
	mas->se.dev = dev;
...
/* SPDX-License-Identifier: GPL-2.0 */
/*
 * Surface SP11X1e BIOS-reference asymmetric polled transaction.
 *
 * Recovered UEFI transaction for qupv3_protocol 0x09:
 *   M_CMD0             = (SPI_TX_RX << 27) | BIT(9) = 0x38000200
 *   TX_TRANS_LEN       = independent TX length
 *   RX_TRANS_LEN       = independent RX length
 *   TX_WATERMARK       = 1
 *   RX_WATERMARK       = 0
 *   CFG_SEQ_START      = 1
 *
 * This code lives inside spi-geni-qcom.c so it can reuse the controller's
 * clocks, ICC votes, runtime PM, packing and FIFO geometry without duplicating
 * platform ownership in a second MMIO driver.
 */
...
static int biosref_prepare_engine(struct spi_geni_master *mas)
{
        ...
        if (!mas->biosref_qspi || geni_se_read_proto(se) != GENI_SE_QSPI)
                return -EOPNOTSUPP;

        for (i = 0; i < 10000; i++) {
                if (!(readl(se->base + SE_GENI_STATUS) &
                      M_GENI_CMD_ACTIVE))
                        break;
                udelay(1);
        }
        ...
        /* Discard any words left by firmware or an interrupted transaction. */
        for (i = 0; i < 256; i++) {
                u32 status = readl(se->base + SE_GENI_RX_FIFO_STATUS);
                               ...
                               u32 speed_hz, unsigned int timeout_ms)
{
	...
	if (!spi_slv || !tx_buf || !tx_len || (!!rx_buf != !!rx_len))
        ...
        /* The normal ISR must not consume status/FIFO state from this command. */
        disable_irq(mas->irq);
        ...
        writel(0, se->base + SE_SPI_TRANS_CFG); /* EFI: no CS toggle */
	...
	geni_se_setup_m_cmd(se, rx_len ? SPI_TX_RX : SPI_TX_ONLY,
        ...
        while (ktime_before(ktime_get(), deadline)) {
                ...
                if (irq_status & BIOSREF_ERR_IRQS) {
                        dev_err(mas->dev,
                                "biosref transfer GENI error %#010x\n",
...
out_restore_irq:
        mas->cur_bits_per_word = 0; /* force normal path to reconfigure */
